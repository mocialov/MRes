...
class NEATPopulation(SimplePopulation):
    ...
    def __init__(self, geno_factory,
                 ...
                 compatibility_threshold_delta=1,
                 target_species=12,
                 min_elitism_size=1,
                 young_age=10,
    ...
    def _status_report(self, id, developer, generation):
        
        a_file = os.path.join(os.path.dirname(__file__), '../../best_solution_'+str(id)+'_generation_'+str(generation))
        while True:
            try:
                with open(a_file, 'a+') as file: # automatically closes...
                    phenotype = developer.convert(self.champions[-1])
                    file.write('fitness: '+str(self.champions[-1].stats['fitness'])+' || ' +' '.join(map(str, phenotype.get_connectivity_matrix()))+'\n')
        ...
        print "\n== Detector %d , Generation %d ==" % (int(id), self.generation)
